...
# intialisation des inputs
    ...
    if mines == sum([i.count("mflag") for i in base_table]) and mines == sum([i.count("F") for i in game_board]) and 0 == sum([i.count("█") for i in game_board]) or any("BOOM" in sublist for sublist in game_board):
        continue_game = False
